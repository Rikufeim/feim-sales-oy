 ...
 * ServicesList
 */
const ServicesList = ({ onStartProject }: { onStartProject?: () => void }) => {
  const services = [{
    title: "NOPEUS",
    icon: Zap,
  ...
  return <section id="services" className="relative py-24 overflow-hidden min-h-[800px] rounded-[2.5rem] mx-2 sm:mx-4 bg-black">
    ...
    <div className="space-y-32 relative z-10">
      {services.map((s, i) => <ServiceRow key={i} index={i} title={s.title} icon={s.icon} description={s.desc} ctaText={s.cta} onStartProject={onStartProject} />)}
    </div>
  </section>;
};
...
const ProjectWizard = ({
  ...
  const handleTypeSelect = (type: string) => {
    ...
    // Only Nettisivut goes to step 2 for sub-selection
  ...
  const renderStep1 = () => <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
      ...
      {/* "Jotain muuta?" card - goes directly to contact form */}
      <button onClick={() => handleTypeSelect('custom')} className="group bg-[#111] p-8 rounded-2xl border border-white/10 hover:border-white/30 text-left transition-all">
        <Zap className="mb-4 text-white" size={32} />
        <h3 className="text-xl font-bold text-white">Prototyypit</h3>
        <p className="text-gray-400 text-sm mt-1">Nopea MVP tai konsepti</p>
      </button>
    </div>
  </div>;

  const renderStep2 = () => <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
    <div className="text-center space-y-4">
      <TypewriterTitle text="Millainen sivu?" className="text-3xl md:text-5xl font-bold text-white" />
    </div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
      <button onClick={() => handleSubTypeSelect('landing')} className="group bg-[#111] p-8 rounded-2xl border border-white/10 hover:border-white/30 text-left transition-all">
        <h3 className="text-xl font-bold text-white mb-2">Landing Page</h3>
        <p className="text-gray-400 text-sm">Yhden sivun myyvä kokonaisuus.</p>
      </button>
      <button onClick={() => handleSubTypeSelect('multipage')} className="group bg-[#111] p-8 rounded-2xl border border-white/10 hover:border-white/30 text-left transition-all">
        <h3 className="text-xl font-bold text-white mb-2">Multipage</h3>
        <p className="text-gray-400 text-sm">Laaja sivusto usealla alasivulla.</p>
      </button>
      <button onClick={() => handleSubTypeSelect('growth')} className="group bg-[#111] p-8 rounded-2xl border border-white/10 hover:border-white/30 text-left transition-all">
        <h3 className="text-xl font-bold text-white mb-2">Growth Package</h3>
        <p className="text-gray-400 text-sm">Sivut + SEO + analytiikka kasvuun.</p>
      </button>
    </div>
  </div>;

  const renderStep3 = () => <div className="animate-in fade-in slide-in-from-right-4 duration-500 w-full max-w-2xl mx-auto">
    <div className="text-center mb-12">
      <TypewriterTitle text="Yhteystiedot" className="text-3xl md:text-5xl font-bold text-white" />
      <p className="text-gray-300 mt-4">Jätä yhteystietosi, niin palaamme asiaan 24 tunnin sisällä.</p>
    </div>
    <form onSubmit={handleContactSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-300 ml-1">Nimi</label>
          <input required type="text" placeholder="Matti Meikäläinen" className="w-full bg-black/50 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-blue-500/50 transition-colors backdrop-blur-sm placeholder-gray-600" />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-300 ml-1">Sähköposti</label>
          <input required type="email" placeholder="matti@yritys.fi" className="w-full bg-black/50 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-blue-500/50 transition-colors backdrop-blur-sm placeholder-gray-600" />
        </div>
      </div>
      <div className="space-y-2">
        <label className="text-sm font-medium text-gray-300 ml-1">
          {selection.type === 'custom' ? 'Kerro ideastasi' : 'Viesti'}
        </label>
        <textarea
          required={selection.type === 'custom'}
          rows={4}
          placeholder={selection.type === 'custom' ? 'Kerro mitä haluaisit rakentaa...' : 'Kerro lyhyesti projektistasi...'}
          className="w-full bg-black/50 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-blue-500/50 transition-colors resize-none backdrop-blur-sm placeholder-gray-600"
        />
      </div>
      <button type="submit" className="w-full bg-white text-black font-bold text-lg py-4 rounded-2xl hover:bg-gray-200 transition-all hover:scale-[1.01] active:scale-[0.99]">
        Lähetä viesti
      </button>
    </form>
  </div>;
  return <div className="min-h-screen flex flex-col relative overflow-hidden bg-black rounded-[2.5rem] mx-2 sm:mx-4 mt-2 sm:mt-4 mb-4">
    ...
    <FloatingDock onNavigate={dest => {
      if (dest === 'home') {
        ...
        else if (step === 3) setStep(selection.type === 'web' ? 2 : 1);
      }
    }} />
...
const Hero = ({
  ...
  return <section id="hero" className="h-[50rem] rounded-[2.5rem] flex flex-col items-start justify-start bg-black antialiased relative overflow-hidden mt-2 sm:mt-4 mx-2 sm:mx-4">
...
const WobbleCardDemo = () => {
  return <section id="wobble" className="relative py-24 px-6 overflow-hidden rounded-[2.5rem] bg-black mx-2 sm:mx-4">
    <div className="absolute inset-0 z-0" style={{
  ...
  return <div className="bg-black min-h-screen font-sans antialiased selection:bg-white/30 selection:text-white">
    {view === 'project' ? <ProjectWizard onBack={handleNavigateHome} onComplete={handleNavigateHome} /> : <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-8 mb-8">
        ...
        <LogoCarouselDemo onStartProject={handleStartProject} />

        <WobbleCardDemo />

        <ServicesList onStartProject={handleStartProject} />
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] mx-2 sm:mx-4 mb-4">
        <div className="absolute inset-0 z-0" style={{
          background: "radial-gradient(circle at 30% 70%, #0021ff70 0%, transparent 45%), radial-gradient(circle at 70% 30%, #2201ff70 0%, transparent 45%), linear-gradient(180deg, #000000 0%, #000000 100%)",
          filter: "brightness(0.6)",
          width: "100%",
